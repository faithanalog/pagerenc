...
pcmEncode (SampleRate sr) (BaudRate br) =
  ...
  where
    ...
    uncons [] = Nothing
    uncons (x:xs) = Just (encodeBit x, xs)
...
noise singleton range count randGen =
  let psi (xs, gen) =
        case randomR range gen of
          (x, nextGen) -> (mappend (singleton x) xs, nextGen)
  in appEndo (stimesMonoid count (Endo psi)) (mempty, randGen)
